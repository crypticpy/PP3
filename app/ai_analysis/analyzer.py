...
class AIAnalysis:
    ...
    def __init__(self,
        ...
        self._analysis_cache: Dict[int, Tuple[datetime,
                                              LegislationAnalysis]] = {}
        self._cache_lock = Lock()

        logger.info(
            f"AIAnalysis initialized with model {self.config.model_name}")
    ...
    def analyze_legislation(self, legislation_id: int) -> LegislationAnalysis:
        ...
        with self._cache_lock:
            if legislation_id in self._analysis_cache:
                cache_time, cached_analysis = self._analysis_cache[
                    legislation_id]
                cache_age_minutes = (datetime.now(timezone.utc) -
                                     cache_time).total_seconds() / 60
        ...
        leg_obj = self.db_session.query(Legislation).filter_by(
        ...
        if text_rec is not None and text_rec.text_content is not None:
            ...
            full_text = self._ensure_plain_string(
                leg_obj.description if leg_obj.description is not None else "")
        ...
        if token_count > safe_limit:
            ...
            chunks, has_structure = self.text_chunker.chunk_text(
                full_text, safe_limit)
            if len(chunks) == 1:
                text_for_analysis = chunks[0]
                analysis_data = self._call_structured_analysis(
                    text_for_analysis)
            else:
                analysis_data = self._analyze_in_chunks(
                    chunks, has_structure, leg_obj)
        else:
        ...
        result_analysis = self._store_legislation_analysis(
            legislation_id, analysis_data)

        with self._cache_lock:
            self._analysis_cache[legislation_id] = (datetime.now(timezone.utc),
                                                    result_analysis)
    ...
    def _call_structured_analysis(self,
                                  text: str,
    ...
    def _analyze_in_chunks(self, chunks: List[str], structured: bool,
                           leg_obj: Union[Legislation, Any]) -> Dict[str, Any]:
        """
        Breaks the text into chunks and analyzes them sequentially. Merges partial analyses.
        """
        description_text = self._ensure_plain_string(
            getattr(leg_obj, 'description', '') or "")
        govt_type_value = str(
            getattr(getattr(leg_obj, 'govt_type', None), 'value', 'unknown'))
        status_value = str(
            getattr(getattr(leg_obj, 'bill_status', None), 'value', 'unknown'))
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                chunk_result = self._call_structured_analysis(chunk_prompt,
                                                              is_chunk=True)
                if not chunk_result:
                    logger.warning(
        ...
        if "summary" in cumulative_analysis and len(chunks) > 1:
            cumulative_analysis["summary"] = self._post_process_summary(
                cumulative_analysis["summary"], len(chunks))
    ...
    def _post_process_summary(self, summary: str, chunk_count: int) -> str:
        ...
        chunk_phrases = [
            "in this section", "this part of the bill",
            "this section of the legislation", "as mentioned earlier",
            "based on the previous sections", "in the previous sections",
    ...
    def _store_legislation_analysis(
            self, legislation_id: int,
            analysis_dict: Dict[str, Any]) -> LegislationAnalysis:
        """
        ...
        with self._db_transaction():
            existing_analyses = self.db_session.query(
                LegislationAnalysis).filter_by(
                    legislation_id=legislation_id).all()

            if existing_analyses:
                # Build a list of integer versions, defaulting to 0 if analysis_version is None.
                versions = [
                    cast(int, x.analysis_version)
                    if x.analysis_version is not None else 0
                ...
                prev = next((x for x in existing_analyses
                             if cast(int, x.analysis_version) == max_version),
                            None)
            ...
            if impact_category_str is not None:
                try:
                    impact_category_enum = ImpactCategoryEnum(
                        impact_category_str)
            ...
            analysis_obj = LegislationAnalysis(
                ...
                public_health_impacts=analysis_dict.get(
                ...
                environmental_impacts=analysis_dict.get(
                    "environmental_impacts", []),
                education_impacts=analysis_dict.get("education_impacts", []),
                infrastructure_impacts=analysis_dict.get(
    ...
    def update_legislation_priority(self, legislation_id: int,
        ...
        with self._db_transaction():
            try:
                ...
                priority_data = calculate_priority_scores(
                    analysis_dict, legislation_id)

                priority = self.db_session.query(
                    LegislationPriority).filter_by(
                        legislation_id=legislation_id).first()
                ...
                if priority:
                    ...
                    if not bool(priority.manually_reviewed):  # Convert to bool
                        ...
                        health_relevance = int(
                            priority_data.get("public_health_relevance") or 0)
                        local_relevance = int(
                            priority_data.get("local_govt_relevance") or 0)
                        overall = int(
                            priority_data.get("overall_priority") or 0)

                        # Option 1: Use setattr
                        setattr(priority, "public_health_relevance",
                                health_relevance)
                        setattr(priority, "local_govt_relevance",
                                local_relevance)
                        setattr(priority, "overall_priority", overall)
                        setattr(priority, "auto_categorized", True)
                        setattr(priority, "auto_categories",
                                priority_data.get("auto_categories", {}))
                    ...
                    new_priority = LegislationPriority(
                        ...
                        auto_categorized=bool(
                            priority_data.get("auto_categorized", False)),
                        auto_categories=priority_data.get(
                            "auto_categories", {}))
    ...
    def get_cached_analysis(
            self, legislation_id: int) -> Optional[LegislationAnalysis]:
        """
    ...
    def get_token_usage_estimate(self, legislation_id: int) -> Dict[str, Any]:
        ...
        if text_rec and text_rec.text_content is not None:
            ...
            if is_binary:
                full_text = self._ensure_plain_string(
                    leg_obj.description if leg_obj.
        ...
        safe_limit = self.config.max_context_tokens - self.config.safety_buffer
        chunks_needed = ((token_count + safe_limit - 1) //
                         safe_limit if token_count > 0 else 1)
    ...
    def batch_analyze(
        ...
        for leg_id in legislation_ids:
            try:
                ...
                results["failed"] += 1
                logger.error(
                    f"Error analyzing legislation ID={leg_id}: {error_msg}")
    ...
    async def _get_async_transaction(self):
        ...
        try:
            ...
            if transaction:
                transaction.rollback()
            logger.error(f"Database error in async transaction: {e}",
                         exc_info=True)
            raise DatabaseError(f"Database operation failed: {str(e)}") from e
        except Exception as e:
            if transaction:
                transaction.rollback()
            logger.error(f"Unexpected error in async transaction: {e}",
                         exc_info=True)
            raise

    async def analyze_legislation_async(
            self, legislation_id: int) -> LegislationAnalysis:
        """
        ...
        async with self._get_async_transaction() as transaction:
            ...
            result_analysis = self._store_legislation_analysis(
                legislation_id, analysis_data)
            with self._cache_lock:
                self._analysis_cache[legislation_id] = (datetime.now(
                    timezone.utc), result_analysis)
    ...
    async def _call_structured_analysis_async(
            ...
            text: str,
            is_chunk: bool = False,
            transaction_ctx: Any = None) -> Dict[str, Any]:
        """
            ...
            if len(chunks) == 1:
                ...
                MockLegislation = type(
                    'MockLegislation', (), {
                        ...
                        "N/A",
                        'title':
                        bill_title or "Unspecified",
                        'description':
                        (safe_bill_text[:500] + "..."
                         if len(safe_bill_text) > 500 else safe_bill_text),
                        'govt_type':
                        MockGovtEnum,
                        'govt_source':
                        "External",
    ...
    async def analyze_bill_async(
        ...
        if token_count > safe_limit:
            ...
            if len(chunks) == 1:
                text_for_analysis = chunks[0]
                return await self._call_structured_analysis_async(
                    text_for_analysis)
            else:
