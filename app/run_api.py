...
async def get_bills(
    ...
    try:
        ...
        query_obj = query_obj.order_by(Legislation.bill_last_action_date.desc())
        ...
        for bill in results:
            bills.append({
                ...
                "title": bill.title,
                "status": bill.bill_status.value if bill.bill_status else "unknown",
                "introduced_date": bill.bill_introduced_date,
                "last_action_date": bill.bill_last_action_date,
                "government_type": bill.govt_type.value if bill.govt_type else "unknown",
...
async def get_bill(bill_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        response = {
            ...
            "has_analysis": bill.analyses is not None and len(bill.analyses) > 0
        }
    ...
    try:
        ...
        if not latest_analysis:
            ...
            try:
                ...
                logger.error(f"Error analyzing bill {bill_id}: {analysis_error}")
                raise HTTPException(status_code=500, detail=f"Error generating analysis: {str(analysis_error)}")
        
        # Format response with all analysis data
        response = {
            "id": latest_analysis.id,
            "legislation_id": latest_analysis.legislation_id,
            "version": latest_analysis.analysis_version,
            ...
            "impact_level": latest_analysis.impact.value if latest_analysis.impact else None,
            ...
            "environmental_impacts": latest_analysis.environmental_impacts,
            "education_impacts": latest_analysis.education_impacts,
            "infrastructure_impacts": latest_analysis.infrastructure_impacts,
            "recommended_actions": latest_analysis.recommended_actions,
            "immediate_actions": latest_analysis.immediate_actions,
            "resource_needs": latest_analysis.resource_needs,
