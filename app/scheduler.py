...
class LegislationSyncManager:
    ...
    def __init__(self, db_session_factory: sessionmaker):
        ...
        self.health_keywords = [
            ...
            "physician", "vaccine", "immunization", "disease", "epidemic", "pandemic",
    ...
    def run_nightly_sync(self) -> Dict[str, Any]:
        ...
        try:
                            ...
                            try:
                                ...
                                if bill_obj:
                                    ...
                                    if "amendments" in bill_data and bill_data["amendments"]:
                                        amendments_count = api._track_amendments(bill_obj, bill_data["amendments"])
            ...
            sync_meta.status = SyncStatusEnum.COMPLETED if not summary["errors"] else SyncStatusEnum.PARTIAL
            ...
            db_session.commit()
            logger.info(f"Nightly sync completed. New bills: {summary['new_bills']}, "
    ...
    def _record_critical_error(self, 
                              db_session: Session, 
                              sync_meta: SyncMetadata, 
                              exception: Exception, 
                              error_type: str) -> None:
    ...
    def _get_active_sessions(self, api: LegiScanAPI, state: str) -> List[Dict[str, Any]]:
        """
    ...
    def _identify_changed_bills(self, db_session: Session, master_list: Dict[str, Any]) -> List[int]:
        """
        ...
        try:
            ...
            for key, bill_info in master_list.items():
                ...
                existing = db_session.query(Legislation).filter(
                    ...
                    Legislation.data_source == "legiscan"  # Using string instead of enum for simplicity
                ).first()
    ...
    def _track_amendments(self, db_session: Session, bill: Legislation, amendments: List[Dict[str, Any]]) -> int:
        """
        ...
        """
        try:
            # Check if the Amendment model exists
            try:
                # Try to import the Amendment model
                from models import Amendment, AmendmentStatusEnum
                has_amendment_model = True
            except ImportError:
                # If Amendment model doesn't exist, we'll store amendment data in raw_api_response
                has_amendment_model = False
                logger.info("Amendment model not found. Storing amendments in raw_api_response.")
            ...
            for amend_data in amendments:
                ...
                if has_amendment_model:
                    ...
                    # Store amendments in raw_api_response if needed
                    # Make sure we don't lose any existing raw_api_response data
    ...
    def seed_historical_data(self, start_date: str = "2025-01-01") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Invalid start date format: {start_date}. Using 2025-01-01.")
        ...
        try:
            ...
            for state in self.target_jurisdictions:
                try:
                    ...
                    for session in all_sessions:
                        ...
                        if session.get("year_start") >= start_datetime.year or session.get("year_end") >= start_datetime.year:
                    ...
                    for session in relevant_sessions:
                        ...
                        logger.info(f"Seeding historical data from {state} session: {session.get('session_name')}")
                        ...
                        try:
                            ...
                            master_list = api.get_master_list(session_id)  # Use full master list
                            if not master_list:
                                ...
                                if not existing:
                                    try:
                                        ...
                                        if bill_obj:
                                            ...
                                            if "amendments" in bill_data and bill_data["amendments"]:
                                                self._track_amendments(db_session, bill_obj, bill_data["amendments"])
                                    except Exception as e:
                    ...
                    try:
                        ...
                        for session_summary in summary["sessions_processed"]:
                            ...
                            if bill:
                                ...
                                if bill_data and bill_data.get("state") == session_summary["state"]:
                                    session_id = bill_data.get("session", {}).get("session_id")
                                    if session_id == session_summary["session_id"]:
            ...
            logger.info(f"Seeding completed. Added {summary['bills_added']} bills, "
            ...
            logger.error(f"Fatal error in historical seeding: {e}", exc_info=True)
            summary["errors"].append(f"Fatal error: {str(e)}")
        finally:
            summary["end_time"] = datetime.now(timezone.utc)
            try:
...
class PolicyPulseScheduler:
    ...
    def _nightly_sync_job(self):
        ...
        try:
            ...
            if len(summary['errors']) > 10:
                logger.warning(f"High error count in nightly sync: {len(summary['errors'])} errors")
    ...
    def _daily_maintenance_job(self):
        ...
        try:
            ...
            deleted_count = db_session.query(SyncError).filter(
            ...
            logger.info(f"Daily maintenance completed. Removed {deleted_count} old error records. VACUUM success: {vacuum_success}")
            db_session.commit()
        except SQLAlchemyError as e:
            logger.error(f"Database error in maintenance job: {e}", exc_info=True)
            db_session.rollback()
        except Exception as e:
